...
def health(_: argparse.Namespace) -> None:
    _, store, embedder, settings = _objects(); _print({"qdrant": store.healthcheck(), "embedding_model": embedder.model, "ollama_url": settings.vector.ollama_url})
...
def index_candidates(args: argparse.Namespace) -> None:
    repo, store, embedder, settings = _objects(); _print(index_candidate_towers(repo=repo, store=store, embedder=embedder, collection_name=args.candidates_collection or settings.vector.candidates_collection, recreate=args.recreate, only_pending=args.only_pending, limit=args.limit))


def match(args: argparse.Namespace) -> None:
    repo, store, embedder, settings = _objects(); _print(match_candidates_from_mongo(repo=repo, store=store, embedder=embedder, jobs_collection=args.jobs_collection or settings.vector.jobs_collection, top_n=args.top_n, output_dir=Path(args.output_dir), limit=args.limit))
...
def build_parser() -> argparse.ArgumentParser:
    ...
    ic = sub.add_parser("index-candidates-from-mongo"); ic.add_argument("--candidates-collection", default=None); ic.add_argument("--recreate", action="store_true"); ic.add_argument("--only-pending", action="store_true"); ic.add_argument("--limit", type=int, default=None); ic.set_defaults(func=index_candidates)
    m = sub.add_parser("match-candidates-from-mongo"); m.add_argument("--jobs-collection", default=None); m.add_argument("--top-n", type=int, default=10); m.add_argument("--limit", type=int, default=None); m.add_argument("--output-dir", default="data/processed/matching"); m.set_defaults(func=match)
    return p
